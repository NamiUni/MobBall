...
public class ItemStackBuilder<B extends ItemStackBuilder<B, M>, M extends ItemMeta> {

    private final ItemStack itemStack;
    private final M itemMeta;

    private static final Component DISABLE_ITALICS = Component.empty()
            .decoration(TextDecoration.ITALIC, false);

    private ItemStackBuilder(final ItemStack itemStack, final M itemMeta) {
        this.itemStack = itemStack.clone();
        this.itemMeta = itemMeta;
    }

    private ItemStackBuilder(final Material material) {
        this(new ItemStack(material));
    }

    @SuppressWarnings("unchecked")
    private ItemStackBuilder(final ItemStack stack) {
        this.itemStack = stack.clone();
        this.itemMeta = (M) stack.getItemMeta();
    }

    private B of(final ItemStack stack, final M meta) {
        return (B) new ItemStackBuilder<B, M>(stack, meta);
    }

    public static <B extends ItemStackBuilder<B, M>, M extends ItemMeta> ItemStackBuilder<B, M> of(final Material material) {
        return new ItemStackBuilder<>(material);
    }

    public static <B extends ItemStackBuilder<B, M>, M extends ItemMeta> ItemStackBuilder<B, M> of(final ItemStack stack) {
    ...
    @SuppressWarnings("unchecked")
    public final B editMeta(final java.util.function.Consumer<M> consumer) {
